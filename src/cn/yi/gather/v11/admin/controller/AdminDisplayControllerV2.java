...
@Controller("adminDisplayControllerV2")
@RequestMapping(value = "v2/admin/display")
public class AdminDisplayControllerV2 {
	...
	public ModelAndView saveHugo(ModelMap modelMap,Indexuserinfo indexuser,Long[] groups){
		...
		indexuser = indexuserinfoServiceV2.indexuserinfoSaveOrUpdate(indexuser); 
		...
		indexorderbyinfoServiceV2.indexorderbyinfoSaveOrUpdate(indexorder);
		modelMap.put("labels", labels);
	...
	public ModelAndView hugoDetail(ModelMap modelMap,String id){
		try {
			List<Long> labelList = new ArrayList<Long>();
			Indexuserinfo indexuser = indexuserinfoServiceV2.findById(id);
			List<Labels> labels = labelsServiceV2.getLabelsByType(0);
	...
	public ModelAndView saveTeam(ModelMap modelMap,Indexteaminfo indexteam,Long[] groups){
		...
		indexteam = indexteaminfoServiceV2.indexteaminfoSaveOrUpdate(indexteam);
	...
	public ModelAndView teaminfoDetail(ModelMap modelMap,String id){
		try {
			List<Long> labelList = new ArrayList<Long>();
			Indexteaminfo teaminfo = indexteaminfoServiceV2.findById(id);
			List<Labels> labels = labelsServiceV2.getLabelsByType(0);
	...
	public void createSpaceshow(HttpServletResponse response,Spaceshow spaceshow,Integer type){
		...
		if(spaceshow.getName().isEmpty()||spaceshow.getTitle().isEmpty()){
			...
			spaceshow = spaceshowServiceV2.spaceshowSaveOrUpdate(spaceshow);
			if(spaceshow ==null){
				jr.setCord(1);
				jr.setMsg("保存失败");
			}else {
	...
	public void updateSpaceshow(HttpServletResponse response,String id,String img,Integer flag){
		...
		if(id==null||id.isEmpty()||img==null||img.isEmpty()||flag==null){
			...
			Spaceshow spaceshow = spaceshowServiceV2.updateSpaceshow(id, img, flag);
			if(spaceshow==null){
				jr.setCord(1);
			}else {
				Set<String> set = new HashSet<String>();
	...
	public ModelAndView joinapplicationList(ModelMap modelMap,com.common.Page<Joinapplication> page,String keyword){
		page = joinapplicationServiceV2.findJoinapplicationList(page.getCurrentPage(), page.getPageSize(), keyword);
