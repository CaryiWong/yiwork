...
$(function () {
    ...
    $switch.on('click', function () {
        ...
        // radio
        var group = $(this).attr('data-switch-group');
        var $switchItem = $('[data-switch-group="' + group + '"]');
        var $radio = $switchItem.filter('input');
        var $switchPanel = $switchItem.filter('[data-switch-value]');
        var value = $radio.filter(':checked').val();

        var $switchPanelShow = $switchPanel.filter('[data-switch-value="' + value + '"]'),
            $switchPanelHide = $switchPanel.not($switchPanelShow);
        $switchPanelShow.removeClass('hide').find('input,select,textarea').removeAttr('disabled');
        $switchPanelHide.addClass('hide').find('input,select,textarea').attr('disabled', 'disabled');
    ...
    if ($date.length) {
        $date.each(function () {
            ...
            if($this.attr('data-datepicker-type') === 'age'){
                customConfig.min = new Date(1900, 0, 1);
            }
            $datePicker = $datePicker.add($this.pickadate(customConfig));
        })
    }
    ...
    function loadBusiness(type) {
        type = type || 0;
        $.ajax(root + 'v20/labels/lablelist', {
            dataType: 'json',
            data: {
                type: 'web',
                labletype:'job',
            ...
            .done(function (data) {
                ...
                type === 0 ? $business.html(options) : $job.html(options).change();
            })
            .fail(function () {
            })
    }
    ...
    var $iframes = $('iframe').load(function () {
        ...
        $iframes.each(function () {
            ...
            if ($this.is('.iframe-response-height')) {
                var height = this.contentDocument.body.clientHeight + 50;
                $this.height(height);
                $(this.contentDocument.body).css('overflow', 'hidden');
            }
        });
        tab.$doms.$contents.filter(':not(.active)').hide();
    });
    //$('iframe')[0].contentDocument.body.clientHeight
