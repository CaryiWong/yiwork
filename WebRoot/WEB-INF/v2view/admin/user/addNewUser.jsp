...
<body>
	<div class="row row-flow member-apply">
		<form class="form-horizontal col-xs-12" name="mainform" id="mainform"
			action="<%=basePath %>v20/admin/user/savenewuser" method="post"
			...
			<div class="row">
				...
				<div class="form-group">
					<label class="col-xs-2 control-label">所属空间</label>

					<div class="col-xs-5">
						<select class="form-control" name="spaceInfo.id">
							 <c:forEach var="la" items="${spaceList}">
							 	<option value="${la.id}">${la.spacename }</option>
							 </c:forEach>
						</select>
					</div>
				</div>
 ...
 --%>
				...
				<div class="form-group">
					<div class="container">
						<button type="submit"
							class="col-xs-3 col-xs-offset-3 btn btn-primary">保存新增</button>
					</div>
				</div>
		</form>
	</div>
