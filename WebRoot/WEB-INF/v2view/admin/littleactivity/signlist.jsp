								...
								<c:if test="${page.totalPage gt 0 }">
									<c:forEach items="${page.result }" var="userinfo" varStatus="vs">
										<tr>
											...
											<td>${userinfo.user.telnum }</td>
											<td>${userinfo.user.email }</td>
											<td>
												<c:if test="${userinfo.user.sex eq 0 }">男</c:if>
												<c:if test="${userinfo.user.sex eq 1 }">女</c:if>
												<c:if test="${userinfo.user.sex eq 2 }">保密</c:if>
											</td>
											<td>
												<c:if test="${userinfo.user.root eq 2 }">是</c:if>
												<c:if test="${userinfo.user.root eq 3 }">否</c:if>
											</td>
											<td>${userinfo.user.unum}</td>
