       ...
       <div class="row row-flow">
            <div class="col-xs-12 member-list">
                <div id="search-condition" class="panel panel-primary search">
             	<form id="mainForm" name="mainForm" action="<%=basePath %>v20/admin/activity/activitylist" method="post">
					...
					<c:if test="${page.totalPage gt 0 }">
							<c:forEach items="${page.result }" var="activity" varStatus="vs">
								<tr>
											...
											<td ><a href="<%=basePath %>v20/admin/activity/getupdateactivity?activityid=${activity.id}">${activity.title}</a></td>
											...
											<td >
												...
												<c:choose>
													<c:when test="${nowDate<activity.opendate}">待开启</c:when>
													<c:when test="${nowDate>activity.opendate && nowDate<action.enddate}">进行中</c:when>
													<c:when test="${nowDate>activity.opendate}">已结束</c:when>
												</c:choose>
											</td>
											...
											<td>
												<c:if test="${activity.checktype eq 0}">待审核</c:if>
												<c:if test="${activity.checktype eq 1}">通过审核</c:if>
												<c:if test="${activity.checktype eq 2}">审核失败</c:if>
											</td>
											<td><fmt:formatDate value="${activity.createdate }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
											<td>
												<c:if test="${activity.onsale eq 0 }"><a class="btn-link btn-sm" href="javascript:void(0);" onclick="setonsale('${activity.id}',1,this);">下架</a></c:if>
												<c:if test="${activity.onsale eq 1 }"><a class="btn-link btn-sm" href="javascript:void(0);" onclick="setonsale('${activity.id}',0,this);">上架</a></c:if>
											</td>
											<%-- <td >
											...
											<td>
												<a class="btn-link btn-sm" href="<%=basePath %>${activity.h5url}?type=apply">
												<%=basePath %>${activity.h5url}?type=apply
												</a>
											</td>
											<td>
												<a class="btn-link btn-sm" href="javascript:void(0);" onclick="pushactivity('${activity.id}');">推送一次</a>
											</td>
										</tr>
						   </c:forEach>
					</c:if>
