...
DragDrop.prototype = {
	//<li id="drag_0" class="drag_li" style="left: 0px; top: 0px;">element_id
    // <div class="v_middle" id="drag_0_h" style="cursor: move;">  elementHeader_id
    //<img alt="1_1.jpg" src="b1.jpg"/>
    //</div>
   // </li>
			...
			success: function(json){
				...
				for(var i=0;i<json.data.length;i++){
					str+='<li id="drag_'+i+'" class="drag_li" style="left: 0px; top: 0px;">';
			      	...
			      	str+='<span class="myspancs">'+json.data[i].iusernickname+'</span>';
			      	str+='</br><img src="'+root+'/download/img?path='+json.data[i].iusermaximg+'" />';
			      	str+='</div></li>';
			      	if(i<25){
			      		tr1+='<td>'+(i+1)+'</td>';
			      		tr2+='<td>'+json.data[i].iusernickname;
			      		...
			      		tr3+='<td>'+(i+1)+'</td>';
			      		tr4+='<td>'+json.data[i].iusernickname;
			      		...
			      		tr5+='<td>'+(i+1)+'</td>';
			      		tr6+='<td>'+json.data[i].iusernickname;
			      		tr6+='<input type="button" value="删除" onclick="delobj(\''+json.data[i].id+'\')"><input type="button" value="修改" onclick="updateobj(\''+json.data[i].id+'\')"></td>';
			      	}else if(i>74&&i<100){
			      		tr7+='<td>'+(i+1)+'</td>';
			      		tr8+='<td>'+json.data[i].iusernickname;
...
function updateobj(id){
	location.href=root+"admin/display/hugodetail?id="+id;
	/* var tmp=root+"admin/display/hugodetail";
		$.ajax({
			url: tmp,
			type: "POST",
			async:false,
...
<div style="position: absolute; bottom: 20px; width: 1200px" align="center">
   ...
   <a href="<%=basePath%>admin/display/createhugo">新增</a>
